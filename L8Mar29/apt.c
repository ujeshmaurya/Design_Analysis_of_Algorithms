...
void detdfslow(node *root,node *currentnode,int dfslow[],int dfsnum[])
{   int flag=0,i;
   	if(root==NULL)
   	  return;
	  ...
	  detdfslow(root->left,currentnode,dfslow,dfsnum);
	  detdfslow(root->right,currentnode,dfslow,dfsnum);
	}
...
void tree(node *root,int dfslow[],int dfsnum[])
{
	if(root!=NULL)
	{
	  detdfslow(root,root,dfslow,dfsnum);
    ...
    for(i=0;i<n;i++)
 	  {  if(adj[data-1][i]==1)
			...
			{ node *child=NULL;
			  ...
			  child=constructnode(i+1);
			  dfsnum[i]=k++;

			  trcreate(parent,child);
